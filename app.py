...
if user_prompt := st.chat_input("Access the Matrix. Engage with Effiong AI..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # Smart Check: If Effiong AI generates a direct link to an mp4 or video pipeline, render it live!
            if ".mp4" in ai_response or "video.pollinations.ai" in ai_response:
                message_placeholder.markdown(ai_response)
                # Parse out video links from response if present to render a native player frame
                import re
            ...
            # Catching the 429 Quota Limit gracefully
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                st.error("⚠️ **Effiong AI Free Neural Core Exhausted:** The daily standard free requests limit has been reached. Please wait for the daily quota reset or attach a billing profile to your Google AI Studio account for uninterrupted access!")
            else:
                st.error(f"Error engaging the intelligence system processor: {e}")
        except Exception as e:
            # Clean catching for the 429 Quota Limit
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                st.error("⚠️ **Effiong AI's Daily Free Neural Core is Exhausted.** The daily free tier request limit has been reached. Please wait a bit or try again tomorrow as the quota resets!")
            else:
                st.error(f"Error engaging the intelligence system processor: {e}")