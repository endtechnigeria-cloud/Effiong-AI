...
# 2. Sidebar configuration for Heritage Window (API Key input removed)
with st.sidebar:
...
# 3. Securely grab the API Key from Cloud Secrets
try:
    api_key = st.secrets["GEMINI_API_KEY"]
except Exception:
    st.error("Missing API Key! Please ensure GEMINI_API_KEY is set up in your Streamlit Cloud Secrets Manager.")
    st.stop()

# Initialize the official Google GenAI client automatically using the cloud secret
client = genai.Client(api_key=api_key)

# Core operational DNA
system_instruction = """
